...
export default function MarketsPage() {
  ...
  return (
    <div className="w-full px-4 md:px-6 xl:px-8 mt-6">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex flex-col md:flex-row md:items-start justify-between mb-8 gap-4">
          ...
          <div className="flex gap-2 overflow-x-auto pb-2 scrollbar-hide md:justify-end">
            {categories.map(cat => (
              <button
                key={cat}
                type="button"
...
function MarketCard({ id, title, description, category, status, resolutionDate, yes, no, volume, options, isAdmin, onDelete }: { id: string, title: string, description?: string, category: string, status?: string, resolutionDate?: string, yes: number, no: number, volume: string, options?: MarketOption[], isAdmin?: boolean, onDelete?: () => void }) {
  ...
  const handleVote = async (optionId: string) => {
    try {
      await api.post('/predictions', { marketId: id, optionId, amount: 10 });
      window.alert('Vote submitted. Refresh to see updated odds.');
    } catch (error) {
      console.error(error);
      ...
      {isAdmin && (
        <div className="absolute top-3 right-3">
          <button
            ...
            className="rounded-md p-2 text-[var(--color-text-secondary)] hover:bg-slate-100"
          >
            <MoreHorizontal className="w-5 h-5" />
          </button>
          {menuOpen && (
            <div className="absolute right-0 top-11 z-10 w-44 rounded-2xl border border-border bg-white p-2 shadow-lg">
              <button
                type="button"
        ...
        <div className="mt-5 space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3 pt-4">
            <button
              type="button"
              disabled={!yesOption}
              onClick={() => yesOption && handleVote(yesOption.id)}
            ...
            <button
              type="button"
              disabled={!noOption}
              onClick={() => noOption && handleVote(noOption.id)}
